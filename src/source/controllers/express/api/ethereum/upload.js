...
const File = require(config.models_dir + '/mongo/file');
...
module.exports = async (req, res) => {
    try {
        ...
        const file = await File.create(req.body.server)
        ...
        contractWithSigner.uploadData(req.body.ether.hash, req.body.ether.price, 2, file._id.toString())
        .then(async (transaction) => {
            ...
            File.updateOne({ _id: file._id }, { $set: { idSolidity: numb } }).exec()
            response_express.success(res, receipt.transactionHash)
            const historyData = {
                senderID: req.token_info._id,
                fileID: file._id,
                contentSender: `Đăng ký bộ data \"${req.body.server.name}\" thành công.`,
                type: 1,
                money: req.body.ether.price,
                descriptionFile: req.body.server.description
            }
